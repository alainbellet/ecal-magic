...
public struct Esp32DeviceState : IInputStateTypeInfo
{
    ...
    public FourCC format => new FourCC('P', 'O', 'T', 'I');
   ...
   // [InputControl(name = "stick/down", parameters = "normalize,normalizeMin=0,normalizeMax=1,normalizeZero=0.5,clamp=2,clampMin=-1,clampMax=0,invert")]
    ...
    {
        return $"{nameof(poti)}: {poti}, {nameof(button)}: {button}";
    }
}
...
public class Esp32Device : InputDevice//, IInputUpdateCallbackReceiver
{
   ...
   // public StickControl stick { get; private set; }
    public AxisControl poti { get; private set; }
    ...
    protected override void FinishSetup()
    {
       ...
       //firstButton = GetChildControl<ButtonControl>("firstButton");
        ...
        poti = GetChildControl<AxisControl>("poti");
    ...
    public static Esp32Device current { get; private set; }
    ...
    #if UNITY_EDITOR
    [MenuItem("Tools/Create Poti Device")]
    ...
    [MenuItem("Tools/Remove Poti Device")]
    private static void RemoveDevice()
    {
        var customDevice = InputSystem.devices.FirstOrDefault(x => x is Esp32Device) as Esp32Device;
  ...
  /* public void OnUpdate()
    {
           ...
           var state = new PotiDeviceState();
           state.main = m.position.x.ReadValue();
           Debug.Log(state);
